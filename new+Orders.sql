...
## question 6:-
select c.customer_id,c.customer_email,concat(c.customer_fname,' ',c.customer_lname) as name,o.order_id,o.product_quantity,p.product_desc,(o.product_quantity*p.product_price) as subtotal
 from online_customer c left join order_header as oh on c.customer_id=oh.customer_id inner join order_items as o on o.order_id=oh.order_id inner join product as p on o.product_id=p.product_id;

## question 7:-
select carton_id,(len*width*height) as carton_volume from carton having carton_volume> (select sum(p.len*p.width*p.height*o.product_quantity) as vol from product as p inner join order_items as o  on p.product_id=o.product_id where o.order_id=10006 group by o.order_id ) order by carton_volume limit 1 ;

## question 8:-
select c.customer_id,concat(c.customer_fname,' ',c.customer_lname)as name ,o.order_id,sum(o.product_quantity) as quantity,oh.order_status
from online_customer c inner join order_header as oh on c.customer_id=oh.customer_id inner join order_items as o on o.order_id=oh.order_id 
 where o.product_quantity>10 and oh.order_status like "shipped"  group by oh.order_id;
 
 ## question 9:-
 select c.customer_id,concat(c.customer_fname,' ',c.customer_lname)as name ,o.order_id,sum(o.product_quantity) as quantity
from online_customer c inner join order_header as oh on c.customer_id=oh.customer_id inner join order_items as o on o.order_id=oh.order_id
where o.order_id>10060 group by o.order_id; 

## question 10:-

select a.country,pc.product_class_desc,sum(o.product_quantity) as total_quantity,(p.product_price*o.product_quantity) as total_value
 from online_customer as c join address as a on c.address_id=a.address_id inner join 
order_header as oh on oh.customer_id=c.customer_id inner join order_items as o on o.order_id=oh.order_id inner join product as p 
on p.product_id=o.product_id inner join product_class as pc on pc.product_class_code=p.product_class_code
where a.country not in ('india','usa') group by a.country order by total_quantity; 